...
export type ArtistReleaseHistory = {
  artists: Record<string, ArtistRelease[]>;
};
...
export const compactArtistName = (value: string) => String(value || '')
  .toLowerCase()
...
const trackingBodyScript = `<script>
  ...
  const sendReleaseAnalyticsEvent = (releaseSlug, artistSlug, eventType) => {
    ...
    const persistentKey = 'lu_release_analytics:' + releaseSlug + ':' + eventGroup + ':' + analyticsBogotaDateKey();
    ...
    const key = eventGroup + ':' + releaseSlug;
...
const trackingSlug = (artist: Artist) => artist.slug.replace(/-/g, '_');

const imageOrPlaceholder = (artist: Artist, className: string, prefix = '') => artist.photo
...
const renderHeroPhoto = (artist: Artist) => artist.photo
...
const renderRelease = (artist: Artist) => {
...
const renderEmbedSection = (title: string, kicker: string, embed: string | undefined, eventName: string) => {
  if (!embed) return '';
...
const renderContact = (artist: Artist) => {
...
export const renderArtistPage = (artist: Artist) => {
  const eventSlug = trackingSlug(artist);
  return `<!DOCTYPE html>
<html lang="es">
<head>
...
export const renderDirectory = (artists: Artist[]) => `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
export const updateSitemapContent = (source: string, artists: Artist[]) => {
...
const visionStarPosition = (index: number) => ({
  top: index * 150 + (index % 2 === 1 ? 70 : 0),
  left: 4 + Math.round((index * 61.8) % 80)
});

const renderConstellationLines = (people: VisionPerson[], height: number) => {
