...
const prefixVisionAsset = path => path.startsWith('http')
  ? path
  : `https://lujourban.com/${path.replace(/^\/+/, '')}`;

const ZAETTA_VISION_PERSON = {
  name: 'ZAETTA',
...
const artistToVisionPerson = artist => ({
...
const renderVisionPersonRow = person => `    <a class="people-row" href="${escapeHtml(person.link)}" data-track-event="casa_gente_click" data-track-label="casa_gente_card" data-track-content="${escapeHtml(person.name)}">
...
const renderVisionCrateItem = (release, index) => `    <a class="crate-cover${index === 0 ? ' is-latest' : ''}" href="${escapeHtml(release.link)}" target="_blank" rel="noopener" data-track-event="casa_catalogo_click" data-track-label="casa_catalogo_cover" data-track-content="${escapeHtml(release.title)}">
