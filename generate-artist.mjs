...
const renderArtistPage = artist => {
  const mainLink = artist.links?.spotify || artist.links?.instagram || artist.links?.tiktok || '/artistas/';
  const hasPrimaryAction = mainLink !== '/artistas/';
  const mainLabel = artist.links?.spotify ? 'Escuchar' : 'Seguir';
  const photo = imageOrPlaceholder(artist, 'artist-profile-photo', '../../');
  const actions = [
    hasPrimaryAction ? `<a href="${escapeHtml(mainLink)}" target="_blank" rel="noopener">${mainLabel}</a>` : '',
    '<a href="/artistas/" class="secondary">Artistas</a>',
    '<a href="/" class="secondary">Lujo Urban</a>'
  ].filter(Boolean).join('\n      ');
...
<header class="artists-nav">
  <a href="/artistas/" class="artists-brand">
    <img src="../../assets/zaetta-music-logo.jpg" alt="Lujo Urban">
    Lujo Urban
  </a>
  <a href="/" class="artists-back">Inicio</a>
</header>

<main>
  <section class="artist-profile fade-up">
    ${photo}
    <h1>${escapeHtml(artist.name)}</h1>
    <p class="artist-role">${escapeHtml(artist.role)} · Lujo Urban</p>
    <p class="artist-bio">${escapeHtml(artist.bio || artist.tagline)}</p>
